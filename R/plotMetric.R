...
plotMetric <- function(
    ...
    if (!metric %in% c(
        ...
        stop(
            paste0(
                'The ',
                metric,
                ' is not suitable. It has to be selected from the gene.pearson.corr, gene.spearman.corr, gene.aov.anova,
    ...
    if (metric %in% c('pcs.vect.corr')) {
        ...
        all.assays.metric <- all.assays.metric %>%
            mutate(pcs = c(1:nb.pc)) %>%
            pivot_longer(
                -pcs,
                names_to = 'datasets',
                values_to = {{metric}}) %>%
    ...
    if (metric %in% c('pcs.vect.corr', 'pcs.lm')) {
        p = p + theme(
            ...
            scale_x_continuous(breaks = (1:nb.pc),
