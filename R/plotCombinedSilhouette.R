...
plotCombinedSilhouette <- function(
    ...
    all.assays.var1 <- lapply(
        ...
        function(x) {
            if (!variable1 %in% names(se.obj@metadata[['metric']][[x]][[paste0('sil.', method)]])) {
                stop(
                    paste0(
                        'The Silhouette based on ',
                        method,
                        ' has not been computed yet for the ',
                        variable1,
                        ' variable and the ',
                        x,
                        ' assay.'
    ...
    all.assays.var1 = all.assays.var1 %>% pivot_longer(everything(),
                                                       names_to = 'datasets',
                                                       values_to = 'sil') %>% mutate(datasets = factor(datasets, levels =
                                                                                                           levels(assay.names)))
    ...
    all.assays.var2 = all.assays.var2 %>% pivot_longer(everything(),
                                                       names_to = 'datasets',
                                                       values_to = 'sil') %>% mutate(datasets = factor(datasets, levels =
                                                                                                           levels(assay.names)))
    ...
    if (length(levels(assay.names)) <= 4) {
        dataSets.colors <- wes_palette(n = length(levels(assay.names)),
                                       name = "GrandBudapest1")[seq(1:length(levels(assay.names)))]
        p = p + scale_color_manual(values = c(dataSets.colors),
                                   name = 'Datasets')
    }
